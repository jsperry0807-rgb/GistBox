...
program.name('gistbox').description('Run your GitHub Gists as CLI tools').version('0.1.0');

program.command('open').description('Open interactive gist selector').action(openCommand);
...
program.command('pin').argument('<gist-id>', 'Gist ID to pin').action(pinCommand);

program.command('unpin').argument('<gist-id>', 'Gist ID to unpin').action(unpinCommand);
